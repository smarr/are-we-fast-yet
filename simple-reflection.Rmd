...
warmup_plot <- function(bench, bench_name) {
...
refl <- droplevels(subset(data, Suite == "reflection" & Iteration >= 50 & Iteration <= 130))
for (vm in levels(refl$VM)) {
  for (bench in levels(refl$Benchmark)) {
    warmup_plot(droplevels(subset(refl, VM == vm & Benchmark == bench)), bench)
  }
}



refl <- droplevels(subset(data, Suite == "reflection" & Iteration > 50))

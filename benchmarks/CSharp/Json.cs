...
public class Json : Benchmark {
  ...
  public override bool VerifyResult(object r)
  {
    ...
    if (!(result.AsObject()?.Get("head")?.IsObject() ?? false)) { return false; }
    if (!(result.AsObject()?.Get("operations")?.IsArray() ?? false)) { return false; }
  ...
  */
  ...
  public class ParseException : Exception {
    ...
    public ParseException(string message, int offset, int line, int column) : base(message + " at " + line + ":" + column) {
      this.offset = offset;
      this.line   = line;
      this.column = column;
    }

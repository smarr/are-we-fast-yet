...
public class Motion {
  ...
  public Motion(final CallSign callsign, final Vector3D posOne,
      final Vector3D posTwo) {
  ...
  private Vector3D delta() {
    return posTwo.minus(this.posOne);
  }

  public Vector3D findIntersection(final Motion other) {
    Vector3D init1 = this.posOne;
    Vector3D init2 = other.posOne;
    Vector3D vec1 = delta();
    Vector3D vec2 = other.delta();
    final double radius = Constants.PROXIMITY_RADIUS;

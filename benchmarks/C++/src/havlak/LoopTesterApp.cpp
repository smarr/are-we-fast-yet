#include "LoopTesterApp.h"

using namespace std;

namespace havlak {

    int LoopTesterApp::buildDiamond(int start) {
        int bb0 = start;
    ...
    void LoopTesterApp::buildConnect(int start, int end) {
        _cfg->addEdge(make_shared<BasicBlockEdge>(_cfg, start, end));
    }

    int LoopTesterApp::buildStraight(int start, int n) {
        for (int i = 0; i < n; i++) {
    ...
    int LoopTesterApp::buildBaseLoop(int from) {
    ...
    void LoopTesterApp::constructSimpleCFG() {
        _cfg->createNode(0);
        buildBaseLoop(0);
        _cfg->createNode(1);
    ...
    vector<int> LoopTesterApp::main(int numDummyLoops, int findLoopIterations, int parLoops,
    ...
    void LoopTesterApp::constructCFG(int parLoops, int pparLoops, int ppparLoops) {
        int n = 2;
    ...
    void LoopTesterApp::addDummyLoops(int numDummyLoops) {
        for (int dummyloop = 0; dummyloop < numDummyLoops; dummyloop++) {
            findLoops(_lsg);
        }
    }

    void LoopTesterApp::findLoops(shared_ptr<LoopStructureGraph> loopStructure) {

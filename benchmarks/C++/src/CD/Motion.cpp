...
namespace CD {
shared_ptr<Vector3D> Motion::delta() const {
  return _posTwo->minus(_posOne);
}

Motion::Motion(shared_ptr<CallSign> callsign,
...
shared_ptr<Vector3D> Motion::findIntersection(shared_ptr<Motion> other) {
  ...
  // this test is not geometrical 3-d intersection test, it takes the fact that
  // the aircraft move into account ; so it is more like a 4d test (it assumes
  // that both of the aircraft have a constant speed over the tested interval)
  ...
  if (a != 0.0) {
    // we are first looking for instances of time when the planes are exactly r
    // from each other at least one plane is moving ; if the planes are moving
    // in parallel, they do not have constant speed
    ...
    if (v1 <= v2 && ((v1 <= 1.0 && 1.0 <= v2) || (v1 <= 0.0 && 0.0 <= v2) ||
      ...
      if (result->_x >= Constants::MIN_X && result->_x <= Constants::MAX_X &&
          result->_y >= Constants::MIN_Y && result->_y <= Constants::MAX_Y &&
          result->_z >= Constants::MIN_Z && result->_z <= Constants::MAX_Z) {
        return result;
      }
    }
  ...
  // the planes have the same speeds and are moving in parallel (or they are not
  // moving at all) they  thus have the same distance all the time ; we

...
namespace CD {
bool CollisionDetector::isInVoxel(shared_ptr<Vector2D> voxel,
                                  shared_ptr<Motion> motion) {
  if (voxel->_x > Constants::MAX_X || voxel->_x < Constants::MIN_X ||
      voxel->_y > Constants::MAX_Y || voxel->_y < Constants::MIN_Y)
    return false;
...
void CollisionDetector::putIntoMap(
...
void CollisionDetector::recurse(
    shared_ptr<RedBlackTree<shared_ptr<Vector2D>,
                            shared_ptr<Vector<shared_ptr<Motion>>>>> voxelMap,
...
shared_ptr<Vector<shared_ptr<Vector<shared_ptr<Motion>>>>>
CollisionDetector::reduceCollisionSet(
    shared_ptr<Vector<shared_ptr<Motion>>> motions) {
  shared_ptr<RedBlackTree<shared_ptr<Vector2D>,
                          shared_ptr<Vector<shared_ptr<Motion>>>>>
      voxelMap =
          make_shared<RedBlackTree<shared_ptr<Vector2D>,
                                   shared_ptr<Vector<shared_ptr<Motion>>>>>();
  motions->forEach([&](shared_ptr<Motion> motion) -> void {
    drawMotionOnVoxelMap(voxelMap, motion);
  });

  shared_ptr<Vector<shared_ptr<Vector<shared_ptr<Motion>>>>> result =
      make_shared<Vector<shared_ptr<Vector<shared_ptr<Motion>>>>>();

  voxelMap->forEach(
      [&result](shared_ptr<
                RedBlackTree<shared_ptr<Vector2D>,
                             shared_ptr<Vector<shared_ptr<Motion>>>>::Entry> e)
          -> void {
        if (e->_value->size() > 1) {
...
shared_ptr<Vector2D> CollisionDetector::voxelHash(
...
void CollisionDetector::drawMotionOnVoxelMap(
    shared_ptr<RedBlackTree<shared_ptr<Vector2D>,
                            shared_ptr<Vector<shared_ptr<Motion>>>>> voxelMap,
    shared_ptr<Motion> motion) {
  shared_ptr<RedBlackTree<shared_ptr<Vector2D>, bool>> seen =
      make_shared<RedBlackTree<shared_ptr<Vector2D>, bool>>();
  recurse(voxelMap, seen, voxelHash(motion->_posOne), motion);
}

CollisionDetector::CollisionDetector() {
  _state =
      make_shared<RedBlackTree<shared_ptr<CallSign>, shared_ptr<Vector3D>>>();
}

shared_ptr<Vector<shared_ptr<Collision>>> CollisionDetector::handleNewFrame(
    shared_ptr<Vector<shared_ptr<Aircraft>>> frame) {
  shared_ptr<Vector<shared_ptr<Motion>>> motions =
      make_shared<Vector<shared_ptr<Motion>>>();
  shared_ptr<RedBlackTree<shared_ptr<CallSign>, bool>> seen =
      make_shared<RedBlackTree<shared_ptr<CallSign>, bool>>();
  frame->forEach([&](shared_ptr<Aircraft> aircraft) -> void {
    shared_ptr<Vector3D> oldPosition =
        _state->putPtr(aircraft->_callsign, aircraft->_position);
    ...
    motions->append(
        make_shared<Motion>(aircraft->_callsign, oldPosition, newPosition));
  });

  shared_ptr<Vector<shared_ptr<CallSign>>> toRemove =
      make_shared<Vector<shared_ptr<CallSign>>>();
  _state->forEach(
      [&seen, &toRemove](
          shared_ptr<RedBlackTree<shared_ptr<CallSign>,
                                  shared_ptr<Vector3D>>::Entry> e) -> void {
        if (!seen->get(e->_key)) {
          toRemove->append(e->_key);
        }
      });

  toRemove->forEach([&](shared_ptr<CallSign> e) -> void { _state->remove(e); });

  shared_ptr<Vector<shared_ptr<Vector<shared_ptr<Motion>>>>> allReduced =
      reduceCollisionSet(motions);
  shared_ptr<Vector<shared_ptr<Collision>>> collisions =
      make_shared<Vector<shared_ptr<Collision>>>();

  allReduced->forEach(
      [&](shared_ptr<Vector<shared_ptr<Motion>>> reduced) -> void {
        for (long unsigned int i = 0; i < reduced->size(); ++i) {
          ...
          for (long unsigned int j = i + 1; j < reduced->size(); ++j) {
            ...
            if (collision != nullptr) {
              collisions->append(make_shared<Collision>(
                  motion1->_callsign, motion2->_callsign, collision));
            }
          }
        }
      });

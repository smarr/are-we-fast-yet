Performance Comparison SOMns and Java
=====================================
...
Overall, SOMns reaches a peak performance of being about `r round(vms_jit[vms_jit$VM=="SOMns-jit",]$RunRatio.geomean, 2)`x
slower than Java, which is also depicted in the following plot.

```{r som-box-overview, echo=FALSE, fig.width=5, fig.height=1.5, dev='svg'}
soms <- droplevels(subset(norm_jit, VM == "SOMns-jit"))

# Rename
name_map <- list("SOMns-jit" = "SOMns")
...
For this comparison, we use `r length(levels(soms$Benchmark))` different
...
somns <- droplevels(subset(norm_jit, VM == "SOMns-jit"))
name_map <- list("SOMns-jit" = "SOMns")
levels(soms$VM)  <- map_names(levels(soms$VM), name_map)
...
table_data <- droplevels(subset(stats_jit, VM == "Java8" | VM == "SOMns-jit"))
name_map <- list("SOMns-jit" = "SOMns",
                 "Java8"     = "Java")

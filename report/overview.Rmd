...
data_fast_vms <- data %>%
  filter(iteration >= 500 & iteration <= 1000 & exe %in% vms_fast) %>%
  droplevels()
...
data_slow_vms <- data %>%
  filter(iteration >= 100 & (exe == "JRubyJ8" | exe == "Pharo")) %>%
  droplevels()
...
All results are normalized to Java 1.8.0_292. Furthermore, we report peak
...
```{r fast-langs-benchmarks, fig.width=6, fig.height=4}
plot_benchmarks_speedup_for_vms(norm_latest, vms_fast)
```

##### Slow Language Implementations

```{r slow-langs-benchmarks, fig.width=6, fig.height=6}

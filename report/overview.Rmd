...
data <- rbind(
  ...
  load_data_url("https://rebench.dev/Are-We-Fast-Yet/data/2899"),
  load_data_url("https://rebench.dev/Are-We-Fast-Yet/data/3059"),
  load_data_url("https://rebench.dev/Are-We-Fast-Yet/data/3062"))

# Hack to separate out the versions
data$commitid <- as.character(data$commitid)
data$commitid[data$commitid =="dc9700"] <- "v23.0"
data$commitid[data$commitid !="v23.0"]  <- "v22.3"

baseline_version <- "v22.3"
...
vm_names <- c(
  ...
  "Java-Temurin-8-int" = "Java-Temurin-8-int",
  "Java-Temurin-17-int" = "Java-Temurin-17-int",
  "Java-Temurin-20-int" = "Java-Temurin-20-int",
  
  "Java-Temurin-8-jit" = "Java-Temurin-8-jit",
  "Java-Temurin-17-jit" = "Java-Temurin-17-jit",
  "Java-Temurin-20-jit" = "Java-Temurin-20-jit",
  
  "Java-OpenJDK-8-jit" = "Java-OpenJDK-8-jit",
  "Java-OpenJDK-8-int" = "Java-OpenJDK-8-int",
  "Java-OpenJDK-17-int" = "Java-OpenJDK-17-int",
  "Java-OpenJDK-17-jit" = "Java-OpenJDK-17-jit",
  
  "CSharp-jit"    = "C# .NET 6.0.200",
 
  "Node-jit"      = "Node.js 20.3",
  "Node-int"      = "Node.js 20.3 jitless", 
  "Bun-jit"       = "Bun 0.6.9",
  "Bun-int"       = "Bun 0.6.9 jitless", 
...
for (e in as.character(unique(data$exe))) {
  if (!(e %in% vms_all)) {
    cat(e)
  }
  assert_that(e %in% vms_all)
}
...
results_peak <- data |>
  compute_all(
    ...
    baseline_vm_peak, baseline_version)

results_peak_node <- data |>
  compute_all(
    iteration >= warmup_jit_iteration | exe %in% vms_int | exe == "CRuby-y-jit",
    "Node-jit", baseline_version)

results_int <- data |>
  compute_all(
    exe %in% vms_int,
    baseline_vm_int, baseline_version)

results_python_int <- data |>
  compute_all(
    exe %in% vms_int,
    "CPython-int", baseline_version)

results_first <- data |>
  compute_all(
    iteration == 1,
    baseline_vm_first, baseline_version)
...
overview_box_plot <- function(stats, vms = NULL, prepare_data = NULL, pre_plot = NULL, new_colors = FALSE, x_breaks = waiver()) {
  ...
  plot <- ggplot(vm_stats, aes(x=ratio, y=reorder(exe, -ratio, FUN = median_fn), fill = exe, group = exeCommitId))
  if (!is.null(pre_plot)) {
    plot <- pre_plot(plot)
  }
...
```{r first-overview, fig.width=8, fig.height=6}
overview_box_plot(
  results_first$stats$bench,
  x_breaks = c(0.7, 0.75, 1, 1.33333, 2, 3, 5, 10, 15, 20, 50, 100, 500, 1000, 250))
```

###### Java Performance

Interpreter 
```{r java-overview-int, fig.width=8, fig.height=2}
...
```{r java-overview-c2, fig.width=8, fig.height=2}
...
overview_box_plot(results_peak_node$stats$bench,
                  c("Node-int", "Node-jit",
                    "Bun-int", "Bun-jit",
                    "GraalJS-HotspotEE-jit", "GraalJS-NativeEE-int"))
...
```{r python-overview-int, fig.width=8, fig.height=1.5}
...
plot_benchmarks_speedup_for_vms(results_peak$normalized, vms_jit)
```

###### Truffle Impl. Only

```{r jit-truffle-benchmarks, fig.width=8, fig.height=5}
plot_benchmarks_speedup_for_vms(results_peak$normalized,
                                c("GraalJS-HotspotEE-jit", 
                                  "GraalPython-HotspotEE-jit",
                                  "TruffleRuby-HotspotEE-jit", 
                                  "TruffleSOM-ast-HotspotCE-jit-main"
))
...
```{r int-truffle-benchmarks, fig.width=8, fig.height=5}
plot_benchmarks_speedup_for_vms(results_int$normalized, c(
  "GraalJS-NativeEE-int",
  "GraalPython-NativeEE-int", 
  "TruffleRuby-NativeEE-int",
  "TruffleSOM-ast-NativeEE-int-main"
...
plot_benchmarks_speedup_for_vms(
  ...
  c("Java-int", "Java8-int", "Node-int","Bun-int", "GraalJS-NativeEE-int"))
```

##### Java+JavaScript JIT Compiler Only
<a id="js-jit"></a>

```{r java-js-benchmarks-jit, fig.width=8, fig.height=2.2}
plot_benchmarks_speedup_for_vms(
  results_peak$normalized,
  c("Java-jit", "Java8-jit", "Node-jit","Bun-jit", "GraalJS-NativeEE-jit"))
```

...
overview_box_plot(stats_j8, c("GraalEnterprise", "GraalJS", "JRubyTruffle", "JRubyTruffleEnterprise", "JRubyTruffleMaster07", "SOMns", "SOMns-Enterprise", "TruffleSOM-TOM-Enterprise"))


overview_box_plot(stats_j8, c("Java8U66", "GraalJS", "JRubyTruffleEnterprise", "SOMns", "TruffleSOM-TOM-Enterprise"))
overview_box_plot(stats_j8, c("Java8U66", "GraalJS", "JRubyTruffle", "SOMns", "TruffleSOM-TOM-Enterprise"))
...
<a id="benoit"></a>
```{r benoits-plot, echo=FALSE, fig.keep='all', fig.width=8, fig.height=3}
## For Benoit
vm_stats <- droplevels(subset(stats_j8, VM %in% c("Java8U66", "JRubyJ8", "Node", "MRI23", "JRubyTruffleEnterprise")))
  vm_stats <- prep(vm_stats)

  plot <- ggplot(vm_stats, aes(x=VM, y=RuntimeFactor, fill = VM)) +
    geom_hline(aes(yintercept=2.25), colour="#333333", linetype="dashed") +
    #geom_hline(aes(yintercept=5), colour="#cccccc", linetype="dashed") +
    geom_boxplot() + #fill=get_color(5, 7)
    theme_bw() + theme_simple() + theme(axis.text.x = element_text(size=16, angle= 90, vjust=0.5, hjust=1),
                                        axis.text.y = element_text(size=16),
                                        legend.position="none") +
    scale_y_continuous(breaks = c(1, 5, 10, 25, 50, 75)) +
    #scale_y_log10(breaks=c(1,2,3,10,20,30,50,100,200,300,500,1000)) + #limit=c(0,30), breaks=seq(0,100,5), expand = c(0,0)
    ggtitle("Runtime Factor, normalized to Java 1.8.0_66 (lower is better)") + coord_flip() + xlab("")
...
plot_benchmarks_speedup_for_vms_faceted(stats, c("GraalEnterprise", "GraalJS", "JRubyTruffleEnterprise", "SOMns", "SOMns-Enterprise", "TruffleSOM-TOM-Enterprise"))
...
vm_stats <- ddply(stats, ~ Benchmark + VM, summarise,
                     geomean = geometric.mean(RuntimeFactor),
                     sd      = sd(RuntimeFactor),
                     min     = min(RuntimeFactor),
                     max     = max(RuntimeFactor),
...
deltablue <- subset(vm_stats, Benchmark == "DeltaBlue")
deltablue$VM <- reorder(deltablue$VM, X=deltablue$geomean)

t <- tabular(Justify("l")*Heading()*VM ~ 
             Heading('Runtime Factor over Java8U66')*Justify("r")*Format(sprintf("%.2f"))*((geomean)*Heading()*identity), data=deltablue)
table_options(justification="c ")
cat("<h3>DeltaBlue</h3>")
html(t)

richards <- subset(vm_stats, Benchmark == "Richards")
richards$VM <- reorder(richards$VM, X=richards$geomean)

t <- tabular(Justify("l")*Heading()*VM ~ 
             Heading('Runtime Factor over Java8U66')*Justify("r")*Format(sprintf("%.2f"))*((geomean)*Heading()*identity), data=richards)
table_options(justification="c ")
cat("<h3>Richards</h3>")
html(t)
```
...
overview_box_plot(stats, c("GraalEnterprise", "JRubyTruffle", "JRubyTruffleEnterprise", "JRubyTruffleMaster07", "Crystal"))
```

```{r jruby-truffle, echo=FALSE, fig.keep='all', fig.width=4, fig.height=16}
plot_benchmarks_speedup_for_vms_faceted(stats, c("GraalEnterprise", "JRubyTruffle", "JRubyTruffleEnterprise", "JRubyTruffleMaster07", "Crystal"))
```
...
```{r som-overview, echo=FALSE, fig.keep='all', fig.width=8, fig.height=3}
prep <- function (d) {
  #d <- prepare_vm_names(d)
  d$VM <- reorder(d$VM, X=-d$RuntimeFactor)
  d
}
overview_box_plot(stats, c("GraalEnterprise", "SOMns", "SOMns-Enterprise", "TruffleSOM", "TruffleSOM-Enterprise", "TruffleSOM-TOM", "TruffleSOM-TOM-Enterprise"), prep)
```


```{r som-benchmarks, echo=FALSE, fig.keep='all', fig.width=2, fig.height=16}
plot_benchmarks_speedup_for_vms_faceted(stats, c("SOMns", "SOMns-Enterprise", "TruffleSOM", "TruffleSOM-Enterprise", "TruffleSOM-TOM", "TruffleSOM-TOM-Enterprise"))
```

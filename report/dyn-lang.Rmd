...
data <- load_data_url("https://rebench.stefan-marr.de/rebenchdb/get-exp-data/1806") %>%
  factorize_result()

# data <-
#   rbind(load_data_url("https://rebench.stefan-marr.de/rebenchdb/get-exp-data/1931"),
#         load_data_url("https://rebench.stefan-marr.de/rebenchdb/get-exp-data/1933"),
#         load_data_url("https://rebench.stefan-marr.de/rebenchdb/get-exp-data/1936")) |>
#   factorize_result()
...
vm_names <- c(
  ...
  "GraalJS-HotspotEE-jit" = "Graal.js EE 22.2 Hotspot", 
  "GraalJS-NativeEE-int"  = "Graal.js EE 22.2 Native Int", 
  
  "PyPy-jit" = "PyPy 3.8 7.3.7",
  "CPython-int" = "CPython 3.10",
  "GraalPython-HotspotEE-jit" = "GraalPython EE 22.2 Hotspot",
  "GraalPython-NativeEE-int"  = "GraalPython EE 22.2 Native Int",
  
  "CRuby-int" = "Ruby 3.1.0",
  "CRuby-y-jit" = "Ruby 3.1.0 yjit",
  "TruffleRuby-HotspotEE-jit" = "TruffleRuby EE 22.2 Hotspot",
  "TruffleRuby-NativeEE-int"  = "TruffleRuby EE 22.2 Native Int",
...
vm_colors[["Graal.js EE 22.2 Hotspot"]]       <- "#c4a000"
vm_colors[["Node.js 17.5 jitless"]]               <- "#edd400"
vm_colors[["Graal.js EE 22.2 Native Int"]]    <- "#fce94f"

vm_colors[["PyPy 3.8 7.3.7"]]                     <- "#2a5703"
vm_colors[["GraalPython EE 22.2 Hotspot"]]    <- "#4e9a06"
vm_colors[["CPython 3.10"]]                       <- "#73d216"
vm_colors[["GraalPython EE 22.2 Native Int"]] <- "#b7f774"

vm_colors[["Ruby 3.1.0"]]                         <- "#3465a4"
vm_colors[["TruffleRuby EE 22.2 Hotspot"]]    <- "#204a87"
vm_colors[["TruffleRuby EE 22.2 Native Int"]] <- "#97c4f0"

vm_colors[["TruffleSOM AST Hotspot CE base"]]     <- "#a40000"
vm_colors[["TruffleSOM AST Hotspot EE base"]]     <- "#a40000"
...
The baseline here is `r vm_names[baseline_vm_peak]`.
Instead of considering all data, let's look strictly at the first iteration.
This would correspond to the performance of one-off scripts, and somewhat small
workloads. However, benchmark sizing is a big issue here, because with it, one
determines whether JIT-compiled or interpreter performance dominates.

```{r first-overview, fig.width=15, fig.height=7, dev.args = list(bg = 'transparent')}
overview_box_plot(results_first$stats$bench, vms = vms_dyn_jit,
  x_breaks = c(0.33, 0.5, 0.75, 1, 1.33, 2, 3, 4, 5, 6, 8),
  font_size = 18,
  x_axis = "Runtime Factor, normalized to Node.js (lower is better)")
```

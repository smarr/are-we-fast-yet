...
# data <-
#   rbind(load_data_file("data/all/136-savina-8c3f6ec5/benchmark.data"),
#         load_data_file("data/all/141-savina-3866adfc/benchmark.data"))

# d2 <- load_data_file("data/all/142-savina-0890cf41/benchmark.data")
# d2$VM <- "SOMns-t-sm2"
# 
# d3 <- load_data_file("data/all/143-savina-26ce00a8/benchmark.data")
# d3$VM <- "SOMns-t-sm3"

# d4 <- load_data_file("data/all/144-savina-463d27a8/benchmark.data")
# d4$VM <- "SOMns-t-sm4"
# 
# d5 <- load_data_file("data/all/146-savina-b9635af9/benchmark.data")
# d5$VM <- "SOMns-t-sid1"
# 
# d6 <- load_data_file("data/all/149-savina-493d113e/benchmark.data")
# d6$VM <- "SOMns-t-sid"
# 
# d7 <- load_data_file("data/all/150-savina-7ba33abd/benchmark.data")
# 
# d8 <- load_data_file("data/all/152-savina-7fe863b7/benchmark.data")
# d8$VM <- revalue(d8$VM, c("SOMns" = "SOMns truf-loop", "SOMns-t" = "Trace truf-loop"))
# 
# d9 <- load_data_file("data/all/154-savina-38b06824/benchmark.data")
# d9$VM <- "Trace, Full Ids probabilistic"
# 
# d10 <- load_data_file("data/all/158-savina-365f7ebf/benchmark.data")
# d10$VM <- "Trace, Full Ids no-stats"
## Ignore old experiments
#data <- subset(data, VM != "SOMns-t-sm" & VM != "SOMns-t-dm")

#data <- rbind(data, d4, d5, d6, d7, d8, d9, d10) # d2, d3,
#data <- load_data_file("data/all/160-savina-fbe9bc5e/benchmark.data")
#data <- load_data_file("data/all/161-savina-fbe9bc5e/benchmark.data")
#data <- load_data_file("data/all/162-savina-6f17944d/benchmark.data")

#data <- load_data_file("data/all/163-savina-be867eb6/benchmark.data")
#data <- load_data_file("data/all/164-savina-f320b4f7/benchmark.data")
#d2 <- load_data_file("data/all/165-savina-0517ac49/benchmark.data")
d2 <- load_data_file("data/006-savina-82874aee.data.bz2")

d2$VM <- revalue(d2$VM, c("SOMns-full-ids" = "SOMns-full-ids-fileout",
                          "SOMns-t-sid" = "SOMns-t-sid-fileout"))
data <- rbind(data, d2)
# data <- d2
...
data <- subset(data, select = c(Value, Unit, Criterion, Benchmark, Suite, VM, Cores, Iteration, version, sha))
trace_data <- droplevels(subset(data, Unit == "byte"))

data <- droplevels(subset(data, Unit != "byte" & Iteration >= 600 & Iteration <= 990))
...
stats <- norm %>%
  group_by(VM, Benchmark, Suite, Cores, version, sha) %>%
  summarise(
    Time.ms = mean(Value),
    sd      = sd(Value),
...
p <- ggplot(stats %>% filter(Suite == "savina-scala" | Suite == "savina-som"), aes(VM, RuntimeFactor))
...
vm_stats <- ddply(stats_latest, ~ VM + Cores, summarise,
...
t <- tabular(Justify("l")*Heading()*VM*Cores ~
             Heading('Runtime Factor over SOMns')*Justify("r")*Format(sprintf("%.2f"))*((geomean + sd + min + max + median)*Heading()*identity), data=vm_stats)
table_options(justification="c ")
html(t)
```

```{r tracing-table, results='asis', echo=FALSE}
vm_stats <- droplevels(subset(stats_latest, VM %in% som_vms & Cores %in% c(1, 8)))
vm_stats <- ddply(vm_stats, ~ VM + Cores, summarise,
                     geomean = geometric.mean(RuntimeFactor),
                     sd      = sd(RuntimeFactor),
                     min     = min(RuntimeFactor),
                     max     = max(RuntimeFactor),
...
```{r benchmarks-c4, fig.width=4, fig.height=16}
plot_benchmarks_speedup_for_vms_faceted(subset(stats_cores_vm, Cores == 4), vms)
```

###### 6 Core

```{r benchmarks-c6, fig.width=4, fig.height=16}
plot_benchmarks_speedup_for_vms_faceted(subset(stats_cores_vm, Cores == 6), vms)
```

###### 8 Core

```{r benchmarks-c8, fig.width=4, fig.height=16}
plot_benchmarks_speedup_for_vms_faceted(subset(stats_cores_vm, Cores == 8), vms)
```
##### SOMns Implementations

###### 1 Core

```{r benchmarks-c1s, fig.width=4, fig.height=16}
plot_benchmarks_speedup_for_vms_faceted(subset(stats_cores_vm, VM %in% som_vms & Cores == 1), vms)
```

###### 2 Core

```{r benchmarks-c2s, fig.width=4, fig.height=16}
plot_benchmarks_speedup_for_vms_faceted(subset(stats_cores_vm, VM %in% som_vms & Cores == 2), vms)
```

###### 4 Core

```{r benchmarks-c4s, fig.width=4, fig.height=16}
plot_benchmarks_speedup_for_vms_faceted(subset(stats_cores_vm, VM %in% som_vms & Cores == 4), vms)
```

###### 6 Core

```{r benchmarks-c6s, fig.width=4, fig.height=16}
plot_benchmarks_speedup_for_vms_faceted(subset(stats_cores_vm, VM %in% som_vms & Cores == 6), vms)
```

###### 8 Core

```{r benchmarks-c8s, fig.width=4, fig.height=16}
plot_benchmarks_speedup_for_vms_faceted(subset(stats_cores_vm, VM %in% som_vms & Cores == 8), vms)
```
...
```{r benchmark-table-c1, results='asis', echo=FALSE}
t_stats <- stats_latest[stats_latest$Cores == 1,]

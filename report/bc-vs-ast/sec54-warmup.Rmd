...
data <- subset(data, exe %in% c("TruffleSOM-native-ast", "TruffleSOM-native-bc", "TruffleSOM-ast-HotspotCE-jit", "TruffleSOM-bc-HotspotCE-jit", "PySOM-ast-jit", "PySOM-bc-jit"))

data$exe <- ifelse(data$exe == "TruffleSOM-ast-HotspotCE-jit", "TruffleSOM-graal-ast", data$exe)
data$exe <- ifelse(data$exe == "TruffleSOM-bc-HotspotCE-jit", "TruffleSOM-graal-bc", data$exe)
...
data$exe <- ifelse(data$exe == "PySOM-ast-jit", "RPySOM-ast-jit", data$exe)
data$exe <- ifelse(data$exe == "PySOM-bc-jit", "RPySOM-bc-jit", data$exe)

data <- subset(data, criterion != "MaxRSS" & suite != "progr-rep-mem")
...
#    scale_color_discrete(label = function (x) parse(text=x), col) +
  ...
  base <- filtered %>%
    compute_baseline("TruffleSOM-graal-ast")
...
pysom_data <- data %>% filter(exe %in% c("RPySOM-ast-jit", "RPySOM-bc-jit"))

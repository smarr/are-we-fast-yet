...
data <- rbind(
  load_data_url("https://rebench.stefan-marr.de/TruffleSOM/data/2848"), # tsom
...
for (b in sorted_benchmarks) { data_b <- data %>% filter(bench == b) %>% droplevels()
  for (e in sorted_exe) { 
    ...
    for (ea in levels(data_e$extraargs)) { data_ea <- data_e %>% filter(extraargs == ea) %>% droplevels()
    
    plot <- warmup_plot(data_ea, b, e)
    print(plot)

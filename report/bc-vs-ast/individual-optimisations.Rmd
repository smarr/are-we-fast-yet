...
compute_stats <- function(data, exe_name, iteration_threshold = 0, baseline_optim="all optimizations") {
  ...
  if (exe_name %in% jit_exe_vals) {
    ...
    # ugly as hell fix for the two missing data points with the new data
    if (exe_name == "RPySOM-ast-jit" || exe_name == "RPySOM-bc-jit") {
      lowered_basic_ops_data <- load_data_for_experiment(2812, "lowered basic ops", "RPySOM") |>
        select(!c(commitid, suite, cmdline, varvalue, cores, expid, runid, trialid, envid, inputsize, extraargs, warmup, invocation)) |>
        filter(exe == exe_name)  %>% filter(criterion == "total")
      lowered_basic_ops_data <- lowered_basic_ops_data %>% filter(iteration >= MIN_JIT_ITERATION_FOR_OLD_DATA)
      data <- rbind(data, lowered_basic_ops_data)
    }
    
    if (exe_name == "TruffleSOM-graal-bc") {
      inline_caching_data <- load_data_for_experiment(2785, "inline caching", "TruffleSOM") |>
        select(!c(commitid, suite, cmdline, varvalue, cores, expid, runid, trialid, envid, inputsize, extraargs, warmup, invocation)) |>
        filter(exe == exe_name)  %>% filter(criterion == "total")
      inline_caching_data <- inline_caching_data %>% filter(iteration >= MIN_JIT_ITERATION_FOR_OLD_DATA)
      data <- rbind(data, inline_caching_data)
    }
...
get_pysom_ast_data <- function() {
  ...
  rbind(load_data_for_experiment(2873, "all optimizations", project_name),
        ...
        load_data_for_experiment(2895, "inline caching", project_name),       
        load_data_for_experiment(2812, "lowered basic ops", project_name) # OLD DATA
        ) %>% filter(criterion == "total")
}
...
macros = c(
    ...
    Cmd("TSOMbcInlineCacheMinX",
        X2((stats_tsom_bc_int |> filter(optim == "inline caching"))$min)),
    ...
    "",
    Cmd("TSOMastJitInlineCacheMedianX",
        X2((stats_tsom_ast_jit |> filter(optim == "inline caching"))$median)),
    Cmd("TSOMastJitInlineCacheMaxX",
        X2((stats_tsom_ast_jit |> filter(optim == "inline caching"))$max)),
    Cmd("TSOMastJitInlineCacheMinX",
        X2((stats_tsom_ast_jit |> filter(optim == "inline caching"))$min)),

    
    Cmd("TSOMbcJitInlineCacheMedianX",
        X2((stats_tsom_bc_jit |> filter(optim == "inline caching"))$median)),
    Cmd("TSOMbcJitInlineCacheMaxX",
        X2((stats_tsom_bc_jit |> filter(optim == "inline caching"))$max)),
    Cmd("TSOMbcJitInlineCacheMinX",
        X2((stats_tsom_bc_jit |> filter(optim == "inline caching"))$min)),

    "",
    Cmd("PySOMastJitInlineCacheMedianX",
        X2((stats_pysom_ast_jit |> filter(optim == "inline caching"))$median)),
    Cmd("PySOMastJitInlineCacheMaxX",
        X2((stats_pysom_ast_jit |> filter(optim == "inline caching"))$max)),
    Cmd("PySOMastJitInlineCacheMinX",
        X2((stats_pysom_ast_jit |> filter(optim == "inline caching"))$min)),

    
    Cmd("PySOMbcJitInlineCacheMedianX",
        X2((stats_pysom_bc_jit |> filter(optim == "inline caching"))$median)),
    Cmd("PySOMbcJitInlineCacheMaxX",
        X2((stats_pysom_bc_jit |> filter(optim == "inline caching"))$max)),
    Cmd("PySOMbcJitInlineCacheMinX",
        X2((stats_pysom_bc_jit |> filter(optim == "inline caching"))$min)),

    "",
    
    Cmd("TSOMastJitInlineStructMedianX",
        X2((stats_tsom_ast_jit |> filter(optim == "inlined control struc."))$median)),
    Cmd("TSOMastJitInlineStructMaxX",
        X2((stats_tsom_ast_jit |> filter(optim == "inlined control struc."))$max)),
    Cmd("TSOMastJitInlineStructMinX",
        X2((stats_tsom_ast_jit |> filter(optim == "inlined control struc."))$min)),
    
    Cmd("PySOMastJitInlineStructMedianX",
        X2((stats_pysom_ast_jit |> filter(optim == "inlined control struc."))$median)),
    Cmd("PySOMastJitInlineStructMaxX",
        X2((stats_pysom_ast_jit |> filter(optim == "inlined control struc."))$max)),
    Cmd("PySOMastJitInlineStructMinX",
        X2((stats_pysom_ast_jit |> filter(optim == "inlined control struc."))$min)),
    
    Cmd("TSOMbcJitInlineStructMedianX",
        X2((stats_tsom_bc_jit |> filter(optim == "inlined control struc."))$median)),
    Cmd("TSOMbcJitInlineStructMaxX",
        X2((stats_tsom_bc_jit |> filter(optim == "inlined control struc."))$max)),
    Cmd("TSOMbcJitInlineStructMinX",
        X2((stats_tsom_bc_jit |> filter(optim == "inlined control struc."))$min)),
    
    Cmd("PySOMbcJitInlineStructMedianX",
        X2((stats_pysom_bc_jit |> filter(optim == "inlined control struc."))$median)),
    Cmd("PySOMbcJitInlineStructMaxX",
        X2((stats_pysom_bc_jit |> filter(optim == "inlined control struc."))$max)),
    Cmd("PySOMbcJitInlineStructMinX",
        X2((stats_pysom_bc_jit |> filter(optim == "inlined control struc."))$min)),
        
    "",
    
    Cmd("PySOMbcJitLowBasicOpsMedianX",
        X2((stats_pysom_bc_jit |> filter(optim == "lowered basic ops"))$median)),
    Cmd("PySOMbcJitLowBasicOpsMaxX",
        X2((stats_pysom_bc_jit |> filter(optim == "lowered basic ops"))$max)),
    Cmd("PySOMbcJitLowBasicOpsMinX",
        X2((stats_pysom_bc_jit |> filter(optim == "lowered basic ops"))$min)),

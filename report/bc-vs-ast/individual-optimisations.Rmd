...
get_pysom_ast_data <- function() {
  project_name <- "RPySOM"
  rbind(load_data_for_experiment(2851, "every optimization", project_name)
        ) %>% filter(criterion == "total")
}
...
only_keep_last_peak_perf_iters <- function(df) {
  LAST_PEAK_ITERS_NBR <- 10
  
  group_cols <- c("bench", "suite", "invocation", "trialid", "optim")

  # define the exe values to group by
  exe_vals <- c("RPySOM-bc-jit", "RPySOM-ast-jit", "TruffleSOM-graal", "TruffleSOM-graal-bc")
  
  # group the data frame by group_cols and exe_vals and find the max iteration number for each group
  max_iters <- df %>%
    filter(exe %in% exe_vals) %>%
    group_by(across(all_of(group_cols)), exe) %>%
    summarize(max_iter = max(iteration))
  
  df_filtered <- df %>%
    left_join(max_iters, by = c(group_cols, "exe")) %>% # join with max_iters to get the max iteration number for each group
    mutate(max_iter = ifelse(is.na(max_iter), 0, max_iter)) %>% # replace NA values in max_iter with 0
    filter(iteration >= (max_iter - LAST_PEAK_ITERS_NBR) | !(exe %in% exe_vals)) # keep only iterations >= max_iter - 10
...
[comment]: ### Unified graph first attempt
```{r yeah-mr-white-science, fig.width=8, fig.height=2.2, echo=FALSE, eval=FALSE}

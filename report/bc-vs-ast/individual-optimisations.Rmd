...
get_data_for_box_plot_optim <- function(input_data, exe_name, baseline_name) {
  df_baseline <- input_data %>% filter(optim == "baseline")
...
box_plot_data <- get_data_for_box_plot_optim(data_optim, "TruffleSOM-native-interp-ast", baseline)
...
box_plot_data <- get_data_for_box_plot_optim(data_optim, "TruffleSOM-graal", baseline)
...
## TSOM data
based_on_what <- load_data_url("https://rebench.stefan-marr.de/TruffleSOM/data/2622") %>% mutate(optim = "baseline")
#no_ast_inline_caching <- load_data_url("https://rebench.stefan-marr.de/TruffleSOM/data/TODOTODO") %>% mutate(optim = "no inline caching")

data_optim <- rbind(based_on_what) %>% filter(criterion == "total")

box_plot_data <- get_data_for_box_plot_optim(data_optim, "TruffleSOM-native-interp-bc", baseline)

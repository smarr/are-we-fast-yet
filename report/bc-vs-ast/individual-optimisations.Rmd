...
box_plot_optim <- function(data, custom_x_breaks=NULL, colors = color_set) {

  color_binding = compute_colors()
...
compute_colors <- function() {
  ...
  num_optim = length(unique_optims)
  optim_colors = viridis::viridis(num_optim)
  names(optim_colors) <- unique_optims
  
  return(optim_colors)
}

color_set <- compute_colors()
...
plot <- box_plot_optim(box_plot_data, colors=color_set)
...
plot <- box_plot_optim(box_plot_data, custom_x_breaks=c(1, 1.5, 1.8), colors=color_set)

...
ggsave(plot, device= cairo_pdf, path=".", filename="optim-ast-tsom-interp.pdf", dpi=300, height=100, width=200, units="mm", family="Arial")
...
no_ast_inline_caching <- load_data_url("https://rebench.stefan-marr.de/RPySOM/data/2657") %>% mutate(optim = "no inline caching")
no_ast_global_caching <- load_data_url("https://rebench.stefan-marr.de/RPySOM/data/2656") %>% mutate(optim = "no global caching")
no_lowered_control_structs <- load_data_url("https://rebench.stefan-marr.de/RPySOM/data/2663") %>% mutate(optim = "no lowered control structures")
...
data_optim <- rbind(based_on_what, no_localnonlocal_vars, no_ast_inline_caching, no_ast_global_caching, no_lowered_control_structs, no_blocks_without_context) %>% filter(criterion == "total")

box_plot_data <- get_data_for_box_plot_optim(data_optim, "RPySOM-ast-interp")

plot <- box_plot_optim(box_plot_data)
#ggsave(plot, device= cairo_pdf, path=".", filename="optim-ast-pysom-interp.pdf", dpi=300, height=100, width=200, units="mm", family="Arial")
...
plot <- box_plot_optim(box_plot_data)
ggsave(plot, device= cairo_pdf, path=".", filename="optim-ast-tsom-jit.pdf", dpi=300, height=100, width=200, units="mm", family="Arial")
...
box_plot_data <- get_data_for_box_plot_optim(data_optim, "RPySOM-ast-jit")

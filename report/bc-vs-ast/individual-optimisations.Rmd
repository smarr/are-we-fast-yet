...
lol_data <- load_data_url("https://rebench.stefan-marr.de/Are-We-Fast-Yet/data/2945")
lol_data$exe <- as.character(lol_data$exe) # otherwise it complained about a factor in the exes which 

get_data_for <- function(name) {
  mdr <- lol_data %>% filter(startsWith(exe, name))
...
data_optim_tsom_ast <- get_data_for("TruffleSOM-ast") #get_tsom_ast_data()
data_optim_tsom_bc <- get_data_for("TruffleSOM-bc") #get_tsom_bc_data()

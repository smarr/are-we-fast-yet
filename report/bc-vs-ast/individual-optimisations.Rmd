...
get_data_for_box_plot_optim <- function(input_data, exe_name, baseline_optim_name="every optimization", iteration_threshold=0) {
  all <- compute_stats(input_data, exe_name, 0)

  # so, we're running out of time to run all the experiments, therefore we also use the old data which uses the TSOM/PySOM AWFY benchmarks
  # and more importantly, doesn't run peak perf for 2k iterations. so, lots of NAs in the final grid, which we remove
  # if all the data was present these filters would be unnecessary
...
get_tsom_ast_data <- function() {
  ...
  rbind(load_data_for_experiment(2848, "every optimization", project_name),
        ...
        load_data_for_experiment(2761, "no supernodes (*)", project_name) # OLD DATA
        ) %>% filter(criterion == "total")
}
...
get_pysom_ast_data <- function() {
  ...
  rbind(load_data_for_experiment(2873, "every optimization", project_name),
        ...
        load_data_for_experiment(2771, "no local/nonlocal vars", project_name), # OLD DATA
        load_data_for_experiment(2764, "no inline caching", project_name), # OLD DATA        
        load_data_for_experiment(2812, "no lowering of unneces. prims", project_name) # OLD DATA
        ) %>% filter(criterion == "total")
}
...
get_pysom_bc_data <- function() {
  ...
  rbind(load_data_for_experiment(2873, "every optimization", project_name),
        ...
        load_data_for_experiment(2764, "no inline caching", project_name), # OLD DATA
        load_data_for_experiment(2774, "no quickening (*)", project_name) # OLD DATA
        ) %>% filter(criterion == "total")
}

# Impact of individual optimisations (ONLY NEW, FINAL DATA SHOWN)
...
all_data_not_final <- load_data_url("https://rebench.stefan-marr.de/Are-We-Fast-Yet/data/2945")
all_data_not_final$exe <- as.character(all_data_not_final$exe) # otherwise it complained about a factor in the exes which 

#""deprecated"", this function should - and will - be removed in time 
get_data_for <- function(name) {
  mdr <- all_data_not_final %>% filter(startsWith(exe, name))
  mdr$optim <- sub("^.*-no-(.*)$", "no-\\1", mdr$exe) # the "exe" column has the optim in the names this time... so we get them from that
  mdr$exe <- sub("^(.*)-no-.*$", "\\1", mdr$exe) # and we rename them to just their real executable names, e.g., "TruffleSOM-ast-int" instead of ""TruffleSOM-ast-int-no-random-optim"
  mdr$optim[mdr$optim == mdr$exe] <- "all optimizations" # if no modifs were done, it's the default case
  mdr
}

get_and_format_new_data <- function(data_url) {
  data <- load_data_url(data_url)
...
data_optim_tsom_ast <- get_and_format_new_data("https://rebench.stefan-marr.de/Are-We-Fast-Yet/data/2957")
data_optim_tsom_bc <- get_and_format_new_data("https://rebench.stefan-marr.de/Are-We-Fast-Yet/data/2980")
data_optim_pysom_ast <- get_data_for("PySOM-ast")# get_pysom_ast_data()
data_optim_pysom_bc <- get_data_for("PySOM-bc") #get_pysom_bc_data()
...
box_plot_data <- get_data_for_box_plot_optim(data_optim_tsom_ast, "TruffleSOM-ast-NativeCE-int")
...
```{r optim-pysom-ast-int, fig.width=8, fig.height=2.2, echo=FALSE, eval=FALSE}
box_plot_data <- get_data_for_box_plot_optim(data_optim_pysom_ast, "PySOM-ast-int")
...
box_plot_data <- get_data_for_box_plot_optim(data_optim_tsom_ast, "TruffleSOM-ast-HotspotCE-jit")
...
```{r optim-pysom-ast-jit, fig.width=8, fig.height=2.2, echo=FALSE, eval=FALSE}
box_plot_data <- get_data_for_box_plot_optim(data_optim_pysom_ast, "PySOM-ast-jit")
...
box_plot_data <- get_data_for_box_plot_optim(data_optim_tsom_bc, "TruffleSOM-bc-NativeCE-int")
...
```{r optim-pysom-bc-interp, fig.width=8, fig.height=2.2, echo=FALSE, eval=FALSE}
box_plot_data <- get_data_for_box_plot_optim(data_optim_pysom_bc, "PySOM-bc-int")
...
box_plot_data <- get_data_for_box_plot_optim(data_optim_tsom_bc, "TruffleSOM-bc-HotspotCE-jit")
...
```{r optim-pysom-bc-jit, fig.width=8, fig.height=2.2, echo=FALSE, eval=FALSE}
box_plot_data <- get_data_for_box_plot_optim(data_optim_pysom_bc, "PySOM-bc-jit")

...
data <- rbind(
  ...
  load_data_url("https://rebench.stefan-marr.de/Are-We-Fast-Yet/data/2897") # JDK20
)

interp_exes <- c("PySOM-bc-int", "Node-int", "PySOM-ast-int", "PyPy-int", "Java8-int", "Java-int", "TruffleSOM-bc-NativeCE-int-astvsbc", "TruffleSOM-ast-NativeCE-int-astvsbc", "Java20-int")
data <- rbind(data %>% filter(!(exe %in% interp_exes)), 
              load_data_url("https://rebench.stefan-marr.de/Are-We-Fast-Yet/data/2893") # interp data
)
...
macros = c(
  ...
  Cmd("SpeediestExeInt", min_exe),
  Cmd("PySOMastIntMedianX", X2((results_int$stats$exe
                             |> filter(exe == "PySOM-ast-int"))$median_ratio)),
  Cmd("PySOMbcIntMedianX", X2((results_int$stats$exe
                            |> filter(exe == "PySOM-bc-int"))$median_ratio)),
  
  Cmd("TSOMastIntMedianX",
      X2((results_int$stats$exe
          |> filter(exe == "TruffleSOM-ast-NativeCE-int-astvsbc"))$median_ratio)),
  Cmd("TSOMbcIntMedianX",
      X2((results_int$stats$exe |>
            filter(exe == "TruffleSOM-bc-NativeCE-int-astvsbc"))$median_ratio)),
  
  Cmd("PySOMastIntMinX",
        X2((results_int$stats$exe |> filter(exe == "PySOM-ast-int"))$min_ratio)),
  Cmd("PySOMastIntMaxX",
        X2((results_int$stats$exe |> filter(exe == "PySOM-ast-int"))$max_ratio)),
  Cmd("PySOMbcIntMinX",
        X2((results_int$stats$exe |> filter(exe == "PySOM-bc-int"))$min_ratio)),
  Cmd("PySOMbcIntMaxX",
        X2((results_int$stats$exe |> filter(exe == "PySOM-bc-int"))$max_ratio)),
  
  Cmd("TSOMastIntMinX",
        X2((results_int$stats$exe |>
              filter(exe == "TruffleSOM-ast-NativeCE-int-astvsbc"))$min_ratio)),
  Cmd("TSOMastIntMaxX",
        X2((results_int$stats$exe |>
              filter(exe == "TruffleSOM-ast-NativeCE-int-astvsbc"))$max_ratio)),
  Cmd("TSOMbcIntMinX",
        X2((results_int$stats$exe |>
              filter(exe == "TruffleSOM-bc-NativeCE-int-astvsbc"))$min_ratio)),
  Cmd("TSOMbcIntMaxX",
        X2((results_int$stats$exe |>
              filter(exe == "TruffleSOM-bc-NativeCE-int-astvsbc"))$max_ratio))
)
...
macros = c(macros,
  ...
  Cmd("NodeJitMedianX", X2((results_peak$stats$exe
                             |> filter(exe == "Node-jit"))$median_ratio)),
  Cmd("NodeJitMinX", X2((results_peak$stats$exe
                            |> filter(exe == "Node-jit"))$min_ratio)),
  Cmd("NodeJitMaxX", X2((results_peak$stats$exe
                             |> filter(exe == "Node-jit"))$max_ratio)),
  
  Cmd("PySOMastJitMedianX", X2((results_peak$stats$exe
                             |> filter(exe == "PySOM-ast-jit"))$median_ratio)),
  Cmd("PySOMbcJitMedianX", X2((results_peak$stats$exe
                            |> filter(exe == "PySOM-bc-jit"))$median_ratio)),
  
  Cmd("TSOMastJitMedianX",
      X2((results_peak$stats$exe
          |> filter(exe == "TruffleSOM-ast-HotspotCE-jit-main"))$median_ratio)),
  Cmd("TSOMbcJitMedianX",
      X2((results_peak$stats$exe |>
            filter(exe == "TruffleSOM-bc-HotspotCE-jit-main"))$median_ratio)),
  
  Cmd("PySOMastJitMinX",
        X2((results_peak$stats$exe |> filter(exe == "PySOM-ast-jit"))$min_ratio)),
  Cmd("PySOMastJitMaxX",
        X2((results_peak$stats$exe |> filter(exe == "PySOM-ast-jit"))$max_ratio)),
  Cmd("PySOMbcJitMinX",
        X2((results_peak$stats$exe |> filter(exe == "PySOM-bc-jit"))$min_ratio)),
  Cmd("PySOMbcJitMaxX",
        X2((results_peak$stats$exe |> filter(exe == "PySOM-bc-jit"))$max_ratio)),
  
  Cmd("TSOMastJitMinX",
        X2((results_peak$stats$exe |>
              filter(exe == "TruffleSOM-ast-HotspotCE-jit-main"))$min_ratio)),
  Cmd("TSOMastJitMaxX",
        X2((results_peak$stats$exe |>
              filter(exe == "TruffleSOM-ast-HotspotCE-jit-main"))$max_ratio)),
  Cmd("TSOMbcJitMinX",
        X2((results_peak$stats$exe |>
              filter(exe == "TruffleSOM-bc-HotspotCE-jit-main"))$min_ratio)),
  Cmd("TSOMbcJitMaxX",
        X2((results_peak$stats$exe |>
              filter(exe == "TruffleSOM-bc-HotspotCE-jit-main"))$max_ratio))
)

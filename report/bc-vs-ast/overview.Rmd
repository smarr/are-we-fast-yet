...
data <- rbind(
  ...
  load_data_url("https://rebench.stefan-marr.de/Are-We-Fast-Yet/data/2800"), # JDK20
...
baseline_vm_peak <- "Java17-C2-jit"
baseline_vm_int <- "Node-int"
baseline_vm_first <- "Java17-C2-jit"
...
plot <- overview_box_plot(results_peak$stats$bench,
                  c("PySOM-ast-jit", "PySOM-bc-jit", 
                    "TruffleSOM-ast-HotspotCE-jit-main",
                    "TruffleSOM-bc-HotspotCE-jit-main", "Java17-C2-jit", "Node-jit"),
                  x_breaks = c(0.5, 0.75, 1, 1.33333, 2, 3, 5, 10, 20))
ggsave("tsom-pysom-peak-perf.pdf", plot, device= cairo_pdf)
plot
```
...
ggsave("tsom-pysom-interp-perf.pdf", plot, device= cairo_pdf)
plot
```

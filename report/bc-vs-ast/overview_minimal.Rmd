---
title: "Performance Overview based on a Reduced Dataset"
...
data <- subset(data, suite != "progr-rep-mem")
...
This following plots providing a basic overview of the performance of Java, Node.js, and our four interpreters.
Normally, for assessing peak performance, one would need to let a VM with JIT compiler run longer to reach a steady state.
However, to keep the overall time for the Getting Started steps in check,
the benchmarks were only run once with only 5 iterations.
Therefore the obtain data is not likely to represent the peak performance of our systems adequately.

## Overview of a minimal run

#### Sec. 5.2 Interpreter Performance, Figure 2
...
#### Sec. 5.3 Peak Performance, Figure 3

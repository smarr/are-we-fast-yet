...
data_repr_tsom <- load_data_url("https://rebench.stefan-marr.de/TruffleSOM/data/2622") %>% 
...
data <- data_repr_tsom %>% filter(exe %in% c("TruffleSOM-native-interp-bc", "TruffleSOM-native-interp-ast"))
...
data <- data_repr_tsom %>% filter(exe %in% c("TruffleSOM-graal", "TruffleSOM-graal-bc"))
data <- data %>% rename(value.x = value) %>% mutate(ratio = 1) # bad code to make it work with the previous boxplot function...

...
# Simple and direct code, no function, no duplication, just executed once
data_rss <- rbind(
  load_data_url("https://rebench.stefan-marr.de/TruffleSOM/data/2597"),
  load_data_url("https://rebench.stefan-marr.de/RPySOM/data/2615")) |>
  filter(criterion == "MaxRSS") |>
      subset(select = -c(commitid, expid, runid, trialid,
                         cmdline, 
                         varvalue, cores, inputsize, extraargs, warmup,
                         iteration, criterion,
                         suite,
                         envid))
...
ggsave(plot, device= cairo_pdf, path=".", filename=paste0(output_folder, "/program-repr-interp-tsom.pdf"), dpi=300, height=50, width=100, units="mm", family="Arial")
...
plot <- box_plot_in_kb(data)
ggsave(plot, device= cairo_pdf, path=".", filename=paste0(output_folder, "/program-repr-jit-tsom.pdf"), dpi=300, height=50, width=100, units="mm", family="Arial")
...
data <- data |> mutate(ratio = 1) # bad code to make it work with the previous boxplot function...

plot <- box_plot_in_kb(data)
ggsave(plot, device= cairo_pdf, path=".", filename=paste0(output_folder, "/program-repr-interp-pysom.pdf"), dpi=300, height=50, width=100, units="mm", family="Arial")
...
plot <- box_plot_in_kb(data)
ggsave(plot, device= cairo_pdf, path=".", filename=paste0(output_folder, "/program-repr-jit-pysom.pdf"), dpi=300, height=50, width=100, units="mm", family="Arial")
...
data_pysom <- data_repr_pysom |>
    filter(bench == "Test") |> mutate(ratio = 1)
